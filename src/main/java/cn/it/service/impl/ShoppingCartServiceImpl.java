...
public class ShoppingCartServiceImpl implements ShoppingCartService {
	...
	public int addItemToCart(int userId, int number, int itemId) {
		...
		if (shoppingCartDao.select(userId) != null) {
			...
			if (cc != null) {
				...
				shoppingCartDao
						.update(s.getCartId(), total, s.getTotalnumber());
			}
	...
	public CartList getAllItem(int userId) {
		...
		if (sh != null) {// 拥有购物车
			...
			for (CartItem cc : c) {
				double total = cc.getTotalPrice();
				BigDecimal total1 = new BigDecimal(total);
				double total2 = total1.setScale(2, RoundingMode.HALF_UP)
						.doubleValue();
				cc.setTotalPrice(total2);
			}
			if (c != null) {
				cl.setSh(sh);
				cl.setCa(c);
	 ...
	 * 提交订单、下单 并删除购物车
	 */
	public Order commitOrder(List<CartItem> ca, int userId, String addr,
			int payAway) {
		Order o = new Order();
