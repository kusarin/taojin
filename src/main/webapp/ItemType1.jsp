...
<body>
	...
	<form action="ItemType1.do?typeh=${typeh}" method="post">
		<!-- 上一页 按钮 -->
		<div align="center">
			<c:choose>
				<c:when test="${page != 1}">
					<a href="ItemType1.do?typeh=${typeh}&page=${page-1}"><input
			...
			<c:forEach items="${pageList}" var="pn">
				<c:choose>
					...
					<c:otherwise>
						<a href="ItemType1.do?typeh=${typeh}&page=${pn}"><U>${pn}</U></a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<!-- 下一页 按钮 -->
			<c:choose>
				<c:when test="${page != totalPage}">
					<a href="ItemType1.do?typeh=${typeh}&page=${page+1}"> <input
						type="button" name="nextPage" value="下一页" class="btn btn-orange" />
					</a>
				</c:when>
				<c:otherwise>
