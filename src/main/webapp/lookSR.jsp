								...
								<div class="mb20">
										...
										<tbody>
											...
											<c:forEach items="${SR}" var="sr">
												<tr>
													...
													<td>${sr.type}</td>
													<td align="center"><a
														href="./findSR.do?id=${sr.shop_id}">查看详情</a></td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
									...
									<div align="center">
										<form action="lookSR.do" method="post">
											<c:choose>
												<c:when test="${page != 1}">
													<a href="lookSR.do?page=${page-1}"><input type="button"
											...
											<c:forEach items="${pageList}" var="pn">
												<c:choose>
													...
													<c:otherwise>
														<a href="lookSR.do?page=${pn}"><U>${pn}</U></a>
													</c:otherwise>
												</c:choose>
											</c:forEach>
											...
											<c:choose>
												<c:when test="${page != totalPage}">
													<a href="lookSR.do?page=${page+1}"> <input
											...
											&nbsp共${totalPage}页 &nbsp -向<input type="text" name="page"
												id="id" onkeyup="this.value=this.value.replace(/\D/g,'')"
												onafterpaste="this.value=this.value.replace(/\D/g,'')" />页 <input
												type="submit" value="跳转" onclick="return check()" />
										</form>
									</div>
									</section>
								</div>
